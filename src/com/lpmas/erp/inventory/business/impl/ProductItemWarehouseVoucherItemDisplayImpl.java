...
public class ProductItemWarehouseVoucherItemDisplayImpl extends WarehouseVoucherItemDisplayBusiness {

	@Override
	public String getWarehouseDetailDisStr(int wvId, int sourceType, int sourceOrderId, Boolean isModify) {
		String templeteName = "productWvDetailDisplay.html";
		...
		for (Integer id : wareIdList) {
			ProductItemBean item = client.getProductItemByKey(id);
			itemDetailMap.put(String.valueOf(id), item);
			hasExpirationMap.put(String.valueOf(id),
					voucherItemBusiness.checkHasExpiration(InfoTypeConfig.INFO_TYPE_PRODUCT_ITEM, item));
		}
		// 如果有保质期管理的，获取对应的保质期天数
