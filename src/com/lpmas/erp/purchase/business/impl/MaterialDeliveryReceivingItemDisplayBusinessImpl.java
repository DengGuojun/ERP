...
public class MaterialDeliveryReceivingItemDisplayBusinessImpl extends DeliveryReceivingDisplayBusiness {

	@Override
	public String getDeliveryReceivingDisplayStr(int poId, int dnId, Boolean isModify, int DeliveryOrReceiving) {
		// 根据是商品还是物料判断使用那个模板
		String templeteName = "materialDeliveryReceivingDetailDisplay.html";
		...
		if (DeliveryOrReceiving == PurchaseOrderItemDisplayConfig.RECEVING_DETAIL) {
			...
			// 收货单时根据收货明细转换MAP
			for (DeliveryNoteItemBean bean : deliveryNoteItemList) {
				MaterialInfoBean materialInfoBean = client.getMaterialInfoByKey(bean.getWareId());
				itemDetailMap.put(String.valueOf(bean.getDnItemId()), materialInfoBean);
				itemTypeMap.put(String.valueOf(bean.getDnItemId()),
						client.getMaterialTypeByKey(materialInfoBean.getTypeId1()).getTypeName());
			}
		} else {
			// 发货单时根据采购明细转换MAP
			for (PurchaseOrderItemBean bean : orderItemList) {
				MaterialInfoBean materialInfoBean = client.getMaterialInfoByKey(bean.getWareId());
				itemDetailMap.put(String.valueOf(bean.getItemId()), materialInfoBean);
