...
<%
	...
	int toType = ParamKit.getIntParameter(request, "toType", 0);
	...
	<div class="article_tit">
		...
		<div class="modify_form">
			...
			<p>
				...
				DataLogContentConvertor helper = new DataLogContentConvertor(WarehouseTransferLogDisplayConfig.CONFIG_VALUE_MAP);
		...
		<div class="div_center">
		     <%if(mode.equals("po")){ %>
			 <input type="button" name="button" id="button" value="返回" onclick="javascript:location.href='WarehouseTransferOperationLogList.do?toId=<%=toId%>&readOnly=<%=readOnly%>&wareType=<%=wareType%>&mode=<%=mode%>'">
		     <%}else{ %>
		     <input type="button" name="button" id="button" value="返回" onclick="javascript:location.href='WarehouseTransferOperationLogList.do?mode=<%=mode%>'">
		     <%} %>
		</div>
</body>
</html>